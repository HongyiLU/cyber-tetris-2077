...
describe('GameEndModal', () => {
    ...
    test('renders null when not visible', () => {
        const { container } = render(_jsx(GameEndModal, { visible: false, result: defaultResult, onRetry: jest.fn(), onBackToTitle: jest.fn() }));
        expect(container.firstChild).toBeNull();
    });
    test('displays game over message', () => {
        const result = { ...defaultResult, isVictory: false };
        render(_jsx(GameEndModal, { ...defaultProps, result: result }));
        const title = screen.getByRole('heading', { level: 2 });
        expect(title).toBeInTheDocument();
        expect(title.textContent).toContain('游戏结束');
    });
    test('displays victory message', () => {
        const result = { ...defaultResult, isVictory: true };
        render(_jsx(GameEndModal, { ...defaultProps, result: result }));
        const title = screen.getByRole('heading', { level: 2 });
        expect(title).toBeInTheDocument();
    ...
    test('calls onRetry when retry button is clicked', () => {
        ...
        render(_jsx(GameEndModal, { ...defaultProps, onRetry: onRetry }));
    ...
    test('calls onNextLevel when next level button is clicked on victory', () => {
        ...
        const result = { ...defaultResult, isVictory: true };
        render(_jsx(GameEndModal, { ...defaultProps, result: result, onNextLevel: onNextLevel }));
    ...
    test('calls onBackToTitle when back button is clicked', () => {
        ...
        render(_jsx(GameEndModal, { ...defaultProps, onBackToTitle: onBackToTitle }));
    ...
    test('has correct CSS classes', () => {
        render(_jsx(GameEndModal, { ...defaultProps }));
