...
describe('GameInfo Component', () => {
    ...
    it('应该为所有特殊方块找到卡牌数据', () => {
      ...
      specialBlocks.forEach(({ type, name }) => {
        const gameState = createMockGameState({
          nextPiece: {
            type,
            shape: GAME_CONFIG.SHAPES[type as keyof typeof GAME_CONFIG.SHAPES],
            position: { x: 0, y: 0 },
            color: GAME_CONFIG.COLORS[type as keyof typeof GAME_CONFIG.COLORS],
          },
        });
  ...
  describe('null 和边界情况', () => {
    it('gameState 为 null 时应该返回 null', () => {
      const { container } = render(<GameInfo gameState={null} />);
    ...
    it('所有在 CARDS 中定义的方块都应该能被找到', () => {
      GAME_CONFIG.CARDS.forEach(card => {
        const gameState = createMockGameState({
          nextPiece: {
            type: card.id,
            shape: GAME_CONFIG.SHAPES[card.id as keyof typeof GAME_CONFIG.SHAPES],
            position: { x: 0, y: 0 },
            color: card.color || GAME_CONFIG.COLORS[card.id as keyof typeof GAME_CONFIG.COLORS],
          },
        });
