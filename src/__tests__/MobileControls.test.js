...
describe('MobileControls', () => {
    ...
    it('渲染所有方向按钮', () => {
        render(_jsx(MobileControls, { ...mockHandlers }));
    ...
    it('不渲染重开按钮（当未提供 onRestart 时）', () => {
        ...
        render(_jsx(MobileControls, { ...handlers }));
    ...
    it('点击暂停按钮触发回调', () => {
        render(_jsx(MobileControls, { ...mockHandlers }));
        const button = screen.getByText('⏸️ 暂停').closest('button');
        fireEvent.click(button);
        expect(mockHandlers.onPause).toHaveBeenCalled();
    });
    it('点击重开按钮触发回调', () => {
        render(_jsx(MobileControls, { ...mockHandlers }));
        const button = screen.getByText('🔄 重开').closest('button');
        fireEvent.click(button);
        expect(mockHandlers.onRestart).toHaveBeenCalled();
    });
    it('disabled 状态下按钮不触发回调', () => {
        render(_jsx(MobileControls, { ...mockHandlers, disabled: true }));
        const button = screen.getByText('⬅️').closest('button');
        fireEvent.mouseDown(button);
        expect(mockHandlers.onMoveLeft).not.toHaveBeenCalled();
    });
    it('应用正确的尺寸类名', () => {
        const { container: small } = render(_jsx(MobileControls, { ...mockHandlers, size: "small" }));
        expect(small.querySelector('.size-small')).toBeInTheDocument();
        const { container: medium } = render(_jsx(MobileControls, { ...mockHandlers, size: "medium" }));
        expect(medium.querySelector('.size-medium')).toBeInTheDocument();
        const { container: large } = render(_jsx(MobileControls, { ...mockHandlers, size: "large" }));
        expect(large.querySelector('.size-large')).toBeInTheDocument();
    });
    it('应用正确的透明度', () => {
        const { container } = render(_jsx(MobileControls, { ...mockHandlers, opacity: 0.7 }));
    ...
    describe('v1.9.3 长按硬降功能', () => {
        ...
        it('长按 500ms 触发连发硬降', () => {
            render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { repeatEnabled: true, repeatInterval: 200 } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
        ...
        it('自定义长按触发时间 200ms', () => {
            render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { triggerTime: 200 } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
        ...
        it('自定义长按触发时间 500ms', () => {
            render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { triggerTime: 500 } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
        ...
        it('自定义连发间隔 100ms', () => {
            render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { repeatEnabled: true, repeatInterval: 100 } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
        ...
        it('禁用长按硬降', () => {
            render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { enabled: false } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
        ...
        it('禁用连发功能', () => {
            render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { repeatEnabled: false } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
        ...
        it('组件卸载时清理计时器', () => {
            const { unmount } = render(_jsx(MobileControls, { ...mockHandlers, longPressConfig: { repeatEnabled: true } }));
            const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
            fireEvent.touchStart(touchArea, {
                touches: [{ clientX: 100, clientY: 100 }],
            });
