...
import { EQUIPMENT_CONFIG } from '../config/equipment-config';
describe('EquipmentSystem', () => {
    ...
    describe('效果计算', () => {
        ...
        test('应该计算伤害加成', () => {
            const damageEquipment = EQUIPMENT_CONFIG.find(eq => eq.type === 'Accessory' && eq.effects.some(e => e.type === 'damageBoost'));
            if (damageEquipment) {
        ...
        test('应该计算伤害减免', () => {
            const defenseEquipment = EQUIPMENT_CONFIG.find(eq => eq.type === 'Head' && eq.effects.some(e => e.type === 'damageReduction'));
            if (defenseEquipment) {
        ...
        test('应该计算生命加成', () => {
            const healthEquipment = EQUIPMENT_CONFIG.find(eq => eq.type === 'Body' && eq.effects.some(e => e.type === 'healthBoost'));
            if (healthEquipment) {
        ...
        test('应该叠加多个装备效果', () => {
            // 装备多个有伤害加成的物品
            const damageItems = EQUIPMENT_CONFIG.filter(eq => eq.effects.some(e => e.type === 'damageBoost')).slice(0, 3);
        ...
        test('伤害减免不应超过 90%', () => {
            ...
            const defenseItems = EQUIPMENT_CONFIG.filter(eq => eq.effects.some(e => e.type === 'damageReduction'));
