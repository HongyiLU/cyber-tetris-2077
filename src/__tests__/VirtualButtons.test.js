...
describe('VirtualButtons', () => {
    ...
    it('应用正确的尺寸类名', () => {
        const { container: small } = render(_jsx(VirtualButtons, { ...mockHandlers, size: "small" }));
        expect(small.querySelector('.size-small')).toBeInTheDocument();
        const { container: medium } = render(_jsx(VirtualButtons, { ...mockHandlers, size: "medium" }));
        expect(medium.querySelector('.size-medium')).toBeInTheDocument();
        const { container: large } = render(_jsx(VirtualButtons, { ...mockHandlers, size: "large" }));
        expect(large.querySelector('.size-large')).toBeInTheDocument();
    });
    it('应用正确的透明度', () => {
        const { container } = render(_jsx(VirtualButtons, { ...mockHandlers, opacity: 0.7 }));
    ...
    it('v1.9.3 - 自定义长按触发时间 200ms', () => {
        render(_jsx(VirtualButtons, { ...mockHandlers, longPressConfig: { triggerTime: 200 } }));
        const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
        fireEvent.touchStart(touchArea, {
            touches: [{ clientX: 100, clientY: 100 }],
        });
    ...
    it('v1.9.3 - 自定义长按触发时间 500ms', () => {
        render(_jsx(VirtualButtons, { ...mockHandlers, longPressConfig: { triggerTime: 500 } }));
        const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
        fireEvent.touchStart(touchArea, {
            touches: [{ clientX: 100, clientY: 100 }],
        });
    ...
    it('v1.9.3 - 自定义连发间隔 100ms', () => {
        render(_jsx(VirtualButtons, { ...mockHandlers, longPressConfig: { repeatEnabled: true, repeatInterval: 100 } }));
        const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
        fireEvent.touchStart(touchArea, {
            touches: [{ clientX: 100, clientY: 100 }],
        });
    ...
    it('v1.9.3 - 禁用长按硬降', () => {
        render(_jsx(VirtualButtons, { ...mockHandlers, longPressConfig: { enabled: false } }));
        const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
        fireEvent.touchStart(touchArea, {
            touches: [{ clientX: 100, clientY: 100 }],
        });
    ...
    it('v1.9.3 - 禁用连发功能', () => {
        render(_jsx(VirtualButtons, { ...mockHandlers, longPressConfig: { repeatEnabled: false } }));
        const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
        fireEvent.touchStart(touchArea, {
            touches: [{ clientX: 100, clientY: 100 }],
        });
    ...
    it('v1.9.3 - 组件卸载时清理计时器', () => {
        const { unmount } = render(_jsx(VirtualButtons, { ...mockHandlers, longPressConfig: { repeatEnabled: true } }));
        const touchArea = screen.getByText('滑动控制 | 长按硬降').parentElement;
        fireEvent.touchStart(touchArea, {
            touches: [{ clientX: 100, clientY: 100 }],
        });
