...
describe('GameStartCountdown', () => {
    ...
    test('renders null when not visible', () => {
        const { container } = render(_jsx(GameStartCountdown, { visible: false, onComplete: () => { } }));
        expect(container.firstChild).toBeNull();
    });
    // 测试 2: 显示时渲染倒计时容器
    test('renders countdown container when visible', () => {
        render(_jsx(GameStartCountdown, { visible: true, duration: 3, onComplete: () => { } }));
        const countdownElement = screen.getByText('3');
        expect(countdownElement).toBeInTheDocument();
    });
    // 测试 3: 显示正确的初始倒计时数字
    test('displays correct initial countdown number', () => {
        render(_jsx(GameStartCountdown, { visible: true, duration: 5, onComplete: () => { } }));
        expect(screen.getByText('5')).toBeInTheDocument();
    });
    // 测试 4: 默认持续时间为 3 秒
    test('uses default duration of 3 seconds', () => {
        render(_jsx(GameStartCountdown, { visible: true, onComplete: () => { } }));
    ...
    test('calls onComplete when countdown reaches 0', async () => {
        ...
        render(_jsx(GameStartCountdown, { visible: true, duration: 3, onComplete: onCompleteMock }));
        // 前进 3 秒
    ...
    test('calls onCancel when clicked', () => {
        ...
        render(_jsx(GameStartCountdown, { visible: true, duration: 3, onComplete: () => { }, onCancel: onCancelMock }));
        // 点击倒计时
    ...
    test('hides countdown after click', () => {
        const { rerender } = render(_jsx(GameStartCountdown, { visible: true, duration: 3, onComplete: () => { }, onCancel: () => { } }));
        expect(screen.getByText('3')).toBeInTheDocument();
        // 模拟 visible 变为 false
        rerender(_jsx(GameStartCountdown, { visible: false, duration: 3, onComplete: () => { }, onCancel: () => { } }));
    ...
    test('respects custom duration', () => {
        render(_jsx(GameStartCountdown, { visible: true, duration: 10, onComplete: () => { } }));
        expect(screen.getByText('10')).toBeInTheDocument();
    });
    // 测试 14: 倒计时过程中 visible 变为 false 时停止
    test('stops countdown when visible becomes false', () => {
        const onCompleteMock = jest.fn();
        const { rerender } = render(_jsx(GameStartCountdown, { visible: true, duration: 3, onComplete: onCompleteMock }));
        // 前进 1 秒
        jest.advanceTimersByTime(1000);
        // 设置 visible 为 false
        rerender(_jsx(GameStartCountdown, { visible: false, duration: 3, onComplete: onCompleteMock }));
        // 再前进 5 秒
    ...
    test('does not render when visible is false', () => {
        const { container } = render(_jsx(GameStartCountdown, { visible: false, duration: 3, onComplete: () => { } }));
        expect(container.firstChild).toBeNull();
    });
    // 测试 16: 装饰元素存在
    ...
    test('is clickable for cancellation', () => {
        render(_jsx(GameStartCountdown, { visible: true, duration: 3, onComplete: () => { }, onCancel: () => { } }));
        const countdownElement = screen.getByText('3');
    ...
    test('starts countdown from specified duration', () => {
        const { rerender } = render(_jsx(GameStartCountdown, { visible: true, duration: 7, onComplete: () => { } }));
        expect(screen.getByText('7')).toBeInTheDocument();
        // 重新渲染，改变 duration
        rerender(_jsx(GameStartCountdown, { visible: true, duration: 2, onComplete: () => { } }));
        expect(screen.getByText('2')).toBeInTheDocument();
    });
});
