...
describe('牌堆模式 + 抽空惩罚集成测试', () => {
  ...
  test('完整流程：抽卡 → 抽空 → 洗牌 → 惩罚', () => {
    ...
    const initialEmptyRows = initialState.board.filter(
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const nonEmptyRowsBefore = stateBefore.board.filter(
        row => row.some(cell => cell !== 0)
      ).length;
      const nonEmptyRowsAfter = stateAfter.board.filter(
        row => row.some(cell => cell !== 0)
      ).length;
  ...
  test('惩罚触发时垃圾行数量正确', () => {
    ...
    (engine as any).triggerGarbagePenalty(
      GAME_CONFIG.DECK.GARBAGE_PENALTY_ROWS
    );
  ...
  test('空卡组时应该使用回退卡牌', () => {
    ...
    const piece = (engine as any).createPiece();
