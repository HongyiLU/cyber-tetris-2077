...
export class EquipmentSystem {
  ...
  private onStateChange?: (state: EquipmentState) => void;

  constructor(initialState?: Partial<EquipmentState>) {
    this.state = {
      slots: {
    ...
    if (!initialState?.unlockedEquipment || initialState.unlockedEquipment.length === 0) {
      const commonEquipment = EQUIPMENT_CONFIG
        .filter((eq: Equipment) => eq.rarity === 'Common')
   ...
   * 设置状态变化回调
   */
  setStateChangeCallback(callback: (state: EquipmentState) => void) {
   ...
   * 装备物品
   */
  equipEquipment(equipmentId: string): boolean {
    const equipment = getEquipmentById(equipmentId);
    if (!equipment) {
      return false; // 装备不存在
    }
   ...
   * 卸下物品
   */
  unequipEquipment(slot: EquipmentType): boolean {
   ...
   * 获取槽位对应的键
   */
  private getSlotForType(type: EquipmentType): 'head' | 'body' | 'accessory' | null {
    switch (type) {
      case 'Head':
        return 'head';
      case 'Body':
   ...
   * 计算当前装备效果
   */
  calculateEffects(): AppliedEquipmentEffects {
    const effects: AppliedEquipmentEffects = {
   ...
   * 重置装备状态（用于新游戏）
   */
  reset(keepUnlocked: boolean = true) {
    this.state.slots = {
      head: null,
      body: null,
      accessory: null,
   ...
   * 从装备 ID 恢复完整对象
   */
  static deserialize(data: string): EquipmentSystem {
    const serializedState = JSON.parse(data) as {
      slots: {
        head: string | null;
        body: string | null;
   ...
   * 从 localStorage 加载
   */
  static loadFromStorage(key: string = 'equipmentState'): EquipmentSystem | null {
    try {
   ...
   * 保存到 localStorage
   */
  saveToStorage(key: string = 'equipmentState'): void {
    try {
