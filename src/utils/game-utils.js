 ...
 */
export const checkCollision = (shape, position, board, cols, rows) => {
 ...
 */
export const getPieceSize = (shape) => {
    // 类型安全修复：检查非零值而非固定值 1
 ...
 * @returns 是否碰撞
 */
export const checkCollisionGhost = (shape, position, board, cols, rows) => {
    for (let row = 0; row < shape.length; row++) {
        for (let col = 0; col < shape[row].length; col++) {
            const cell = shape[row][col];
            if (cell !== 0 && cell !== undefined) {
